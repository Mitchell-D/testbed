...
def plot_keras_loss(csv_path:Path, title:str=None):
...
if __name__=="__main__":
    ...
    csv_path = Path("data/models/ds2s_1/run_2/ds2s_1_prog.csv")
    #'''

    plot_keras_loss(csv_path, "DS2S Run 2 Loss")
    exit(0)

    #data_path = Path("data/models/osmh_1/osmh_1_curated.pkl")
    data_path = Path("data/models/osmh_1/osmh_1_curated.pkl")
    #model_path = Path("data/models/osmh_1/run_1/osmh_1_model.keras")
    #model_path = Path("data/models/ds2s_1/run_1/ds2s_1_27_0.00.hdf5")
    #model_path = Path("data/models/ds2s_1/run_1/ds2s_1_model.keras")
    #model_path = Path("data/models/ds2s_1/run_2/ds2s_1_model.keras")
    model_path = Path("data/models/ds2s_1/run_2/ds2s_1_64_0.00.hdf5")
    ...
    fct = np.stack([
        data_dict[train_or_validate]["X"][i+1:i+horizon+1 , -1 , :-1]
        for i in range(data_dict[train_or_validate]["X"].shape[0]-horizon)
        ])
    feats = data_dict[train_or_validate]["X"][:-horizon]
    truth = np.squeeze(data_dict[train_or_validate]["Y"][:-horizon])
    times = data_dict[train_or_validate]["t"][:-horizon]

    ymean = data_dict["means"][-1]
    ystdev = data_dict["stdevs"][-1]
    mae = np.sum(np.abs(model.predict([feats,fct])-truth))/feats.shape[0]
    print(f"{train_or_validate} MAE: {mae*ystdev}")
