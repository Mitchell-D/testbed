...
:@module eval_grids: This module contains methods and generators for evaluating
    models over a spatial domain, and storing the results as hdf5 files.
    ...
    them as a new HDF5 file ordered like (N, P, S, F) for N chronological runs
    of the model over each of P pixels in the grid. Each run has a sequence
    length S, and each array type has its own F features.

    The prediction grid hdf5 files generated by grid_preds_to_hdf5 can be
    chronologically generated again by gen_grid_prediction_combos, which is
    utilized by bulk_grid_error_stats_to_hdf5 to create grids of bulk
    statistics per each of the N model run time steps.
"""
import numpy as np
